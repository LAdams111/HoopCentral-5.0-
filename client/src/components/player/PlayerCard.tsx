...
export function PlayerCard({
  ...
  return (
    ...
    >
      <div className="relative flex min-h-full cursor-pointer flex-col overflow-hidden rounded-xl border border-border bg-card transition-all duration-300 hover:-translate-y-1 hover:border-primary/50 hover:bg-card/80 hover:shadow-lg hover:shadow-primary/5">
        <div className="relative aspect-[4/5] flex-shrink-0 overflow-hidden bg-muted">
          ...
          <div className="absolute bottom-2 left-2 z-20 md:bottom-4 md:left-4">
            <span className="rounded-sm bg-primary px-1.5 py-0.5 font-bold tracking-wider text-white text-[10px] md:px-2.5 md:text-xs">
              {formatPosition(player.position)}
            </span>
          </div>
        </div>

        <div className={`relative z-20 flex flex-1 flex-col justify-between gap-1 ${compact ? "p-2" : "gap-3 p-2 md:p-5"}`}>
          <div className="min-h-0">
            <h3 className={`font-display leading-tight text-foreground transition-colors group-hover:text-primary ${compact ? "text-sm md:text-base" : "text-lg md:text-xl"}`}>
              {player.name}
            </h3>
