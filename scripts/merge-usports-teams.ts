...
async function main(): Promise<void> {
  ...
  const { getUsportsTeamAliasReportPath, loadUsportsTeamAliasReport } = await import(
    "../server/src/utils/usports-team-aliases.js"
  );
  const {
    applyUsportsCanonicalIdentityUpdates,
    findUsportsCanonicalIdentityUpdates,
    findUsportsDuplicateMergePlans,
    mergeAllUsportsDuplicateTeams,
  } = await import("../server/src/services/merge-teams.service.js");

  const reportPath = getUsportsTeamAliasReportPath();
  const report = loadUsportsTeamAliasReport(reportPath);
  console.log(`Using alias report: ${reportPath}`);
  console.log(
    `  ${Object.keys(report.aliasMap).length} aliasMap entries, ${report.allTeams.length} teams in inventory`,
  );

  const plans = await findUsportsDuplicateMergePlans();
  const identityUpdates = await findUsportsCanonicalIdentityUpdates();

  if (plans.length === 0 && identityUpdates.length === 0) {
    console.log("No U Sports duplicate teams or alias renames found.");
  ...
  console.log("");
  if (plans.length > 0) {
    const results = await mergeAllUsportsDuplicateTeams();
  ...
  const appliedRenames = await applyUsportsCanonicalIdentityUpdates();
